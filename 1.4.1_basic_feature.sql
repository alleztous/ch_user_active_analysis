...
SELECT
/*
device_id,city,province,os,os_version,
province,app_version,model,brand,resolution,app_name
*/
    ...
    when city in ('香港','澳门','台北') then 7
    else 0 end as city,  
    
    case 
    when province is null or province = '' then -1
    when province in ('北京', '江苏', '上海', '天津') then 1
    when province in ('内蒙古', '浙江', '广东', '福建', '山东') then 2
    when province in ('重庆', '湖北', '陕西', '辽宁', '吉林') then 3 
    when province in ('宁夏', '湖南', '海南', '河南', '河北', '四川') then 4
    when province in ('青海','江西','安徽','新疆','西藏','山西','贵州','广西','黑龙江','云南','甘肃') then 5
    when province = '香港' then 6 when province = '澳门' then 7 when province = '台湾' then 8
    else 0 end as province,  -- 给省份分级别(1,2,3,4,5代表了发达程度)，特别将特别贫穷省份列出来了。   
...
/*
    case
    when os is null or os = '' then -1
    when os = 'android' then round(cast(substr(os_version, 1, 3) as double) + 3, 1) 
    when os = 'ios' then round(cast(substr(os_version, 1, 3) as double), 1) 
    end 0 as os_version,  --手机操作系统版本    
*/
    case
    when resolution is null or resolution = '' then -1
    when substr(resolution, 1, 2) in ('27', '26', '25', '24', '23', '22', '21', '20') then 1
    when substr(resolution, 1, 2) in ('19', '18', '17', '16') then 2
    when substr(resolution, 1, 2) in ('15', '14', '13', '12', '11', '10') then 3
    when substr(resolution, 1, 2) in ('98', '97', '96', '95', '94', '93', '92') then 4
    when substr(resolution, 1, 2) in ('89', '88', '87', '86', '85', '84', '80') then 5 
    when substr(resolution, 1, 2) in ('78', '55', '64') then 6
    else 0 end as resolution,  --手机分辨率

    case 
    when app_version is null or app_version = '' then -1
    when app_version like '%.%' then round(cast(substr(app_version, 1, 3) as double), 1)
    else 0 end as app_version,  --app版本

    case 
    when brand is null or brand = '' then -1
    when brand in ('华为','荣耀') then 1
    when brand = 'oppo' then 2 
    when brand = 'vivo' then 3
    when brand = '小米' then 4 
    when brand = '苹果' then 5
    when brand = '三星' then 6
    when brand = '中兴' then 7 
    WHEN brand = '魅族' then 8 
    when brand in ('乐视','金立','索尼') then 9
    else 0 end as brand,
    
    case
    when model is null or model = '' then '未知'
    else model end as model,    
    
    case  -- 区分不同的频道来源
    when channel is null or channel = '' then -1
    when channel = 'AppStore' then 1 when channel = 'CP付费' then 2
    when channel = '主流商店' then 3  when channel = '厂商商店' then 4 
    when channel = '厂商预装' then 5 when channel = 'Wap产品' then 6
    when channel = '展示类广告' then 7 when channel = '搜索引擎广告' then 8
    when channel = '方案商' then 9 when channel = '越狱商店' then 10
    when channel = '运营商' then 11 when channel = '刷机商' then 12 
    else 0 end as channel_category,   -- 频道来源(重新定义类别变量)

    case                              -- 用户年龄离散化 
    when age is null or age = '' then -1
    when age = '-18' then 1 
    ...
    case                              -- 用户性别离散化
    when gender is null or gender = '' then -1
...
from
    ...
    left join (
        ...
        SELECT
            channel_name,
            app_name,
            channel_account_subclass as channel
        FROM
